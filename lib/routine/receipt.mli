...
(** Typed routine receipts.

    A routine's durable record is an append-only JSONL log, one line per
...
(** Alert transitions — the alertable moments of an unattended run.
    ...
    This is also the whole vocabulary a routine's [notify.on] member may
...
(** Budget meters — the per-routine admission fences. *)
module Meter : sig
  type t =
    ...
    | Reaped of {
        ...
        usd : float option;
            (** The run's derived cost; [None] when no rate priced a spent
                lane, which degrades the routine to its run-count fence. *)
...
type t = {
  ...
  digest : string;
      (** The routine policy digest in force when the receipt was written,
...
(** [reap_recorded ~digest ~identity receipts] is [true] iff [receipts]
    ...
    fold under the routine's fire lock and yields, keeping the record to
...
module Pending : sig
  type t = {
    identity : string;  (** The triggering event's identity string. *)
    digest : string;
        (** The routine policy digest the run was spawned under. *)
...
(** [pending_runs receipts] is the runs whose record is open: every spawned
    ...
    routine digest and event identity, in log order. Pairing is by digest
