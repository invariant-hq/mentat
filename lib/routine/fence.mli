...
(** Budget fences — pure folds over a routine's receipts.
    ...
    evaluation: editing the routine's policy re-stamps subsequent receipts
    and thereby resets every window, which is how an owner re-admits a
    fenced routine deliberately. *)
...
val effective_runs_per_hour :
  budget:Routine.Budget.t -> trigger:[ `Cli | `Webhook ] -> int option
...
(** [spend_in_window ~digest ~now receipts] is the summed cost of reaped
    ...
    ([usd = None]) contributes nothing — which is why a routine whose model
