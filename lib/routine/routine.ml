  ...
  if Char.equal s.[0] '/' then
    ...
    if
      List.exists
        ...
        components
    then error ~context "must not traverse outside the routine directory"
    else if String.equal (List.hd components) "secrets" then
      error ~context "must not live under secrets/"
    else if String.equal s "routine.json" || String.equal s "ingress.id" then
      error ~context "must not name a reserved routine file"
    else Ok s
...
(* GitHub's documented pull_request webhook actions. Closed like every other
   ...
   routine that silently fires on nothing; a new GitHub action becomes
...
let decode_budget json =
  ...
  match json with
  | Jsont.Object (mems, _) ->
      let* slots = route ~context ~names:[ "per_run"; "per_routine" ] mems in
      let* wall_clock =
      ...
      let* usd_per_day, runs_per_hour =
        match optional slots "per_routine" with
        | None -> Ok (None, None)
        | Some (Jsont.Object (mems, _)) ->
            let context = "budget.per_routine" in
            let* slots =
        ...
        | Some _ -> error ~context:"budget.per_routine" "must be a JSON object"
      in
...
let decode bytes =
  ...
  | Ok (Jsont.Object (mems, _)) ->
      ...
      let* () =
        ...
        | Some (Jsont.Object (mems, _)) ->
            ...
            | None -> Ok ()
            | Some _ ->
                (* No reaper exists to honor the knob; an accepted no-op
                   grant would be worse than the refusal. *)
                error ~context:"retention.keep_failed_worktrees"
                  "parses but is refused as unimplemented")
  ...
  | Ok _ -> error ~context:"" "routine must be a JSON object"

let policy_digest ~routine_json ~prompt ~output_schema =
  Mentat_digest.key ~length:16 ~domain:"mentat.routine.policy.v1"
    [ routine_json; prompt; output_schema ]
