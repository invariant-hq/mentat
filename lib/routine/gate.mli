...
(** The admission gate — is this delivery worth a run?
    ...
    A pure predicate over a routine's webhook arm and a decoded
...
val evaluate :
  repo:string -> Routine.Trigger.Webhook.t -> Event.Pull_request.t -> verdict
(** [evaluate ~repo webhook event] applies [webhook]'s gate to [event],
    checking in order: the delivery's repository is [repo] — the routine's
