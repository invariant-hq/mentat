...
let evaluate ~repo (webhook : Routine.Trigger.Webhook.t)
    (event : Event.Pull_request.t) =
  let { Routine.Trigger.Webhook.events; gate } = webhook in
  let { Routine.Gate.base; drafts; associations } = gate in
  let {
  ...
  if not (String.equal event_repo repo) then
    Skip
      (Printf.sprintf "repository %S is not the routine's %S" event_repo repo)
  else if not (List.mem event_name events) then
    Skip (Printf.sprintf "event %S is not in the routine's events" event_name)
  else
    match base with
