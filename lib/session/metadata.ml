...
module Goal = struct
  ...
  module Claim = struct
    ...
    (* Tolerant by law: a claim the steward cannot read is an absent claim
       (continue), never a fault — so the read walks the raw JSON instead of
       a strict codec whose decode error would need swallowing anyway. *)
    let of_json json =
      match json with
  ...
  let framing_prefix ~objective =
    "Continuing toward the goal: " ^ objective ^ "\n"

  let continuation ~objective =
    framing_prefix ~objective
    ^ "\n\
       Keep working toward it. End this turn by declaring goal_status: \
       status \"done\" only when the goal is genuinely reached, otherwise \
       \"continuing\" with a one-line note on what remains."

  let framing_head = "Continuing toward the goal: "

  let framed_objective text =
    if not (String.starts_with ~prefix:framing_head text) then None
    else
      let start = String.length framing_head in
      match String.index_from_opt text start '\n' with
      | Some stop -> Some (String.sub text start (stop - start))
      | None -> None

  let continuations ~objective turns =
    List.fold_left
      (fun count turn ->
        match Turn.Input.text (Turn.input turn) with
        | Some text
          when Option.equal String.equal (framed_objective text)
                 (Some objective) ->
            count + 1
        | Some _ | None -> count)
      0 turns

  let decide t ~finished ~claim ~continuations ~spent =
