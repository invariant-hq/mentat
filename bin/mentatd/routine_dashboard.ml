...
module Observed = struct
  ...
  type routine = {
    loaded : Routine_store.Loaded.t;
    receipts : (Receipt.t list, Routine_store.Error.t) result;
    runs : run list;
  }
...
(* The observation. Every read is per request — the roster, each routine's
...
let observe ~dirs ~store =
  Result.map
    (List.map (fun (name, entry) ->
         ...
         | Ok loaded ->
             let receipts = Routine_store.read_receipts dirs ~name in
             let runs =
               match receipts with
               | Error _ -> []
               | Ok receipts ->
             ...
             in
             Observed.Routine { Observed.loaded; receipts; runs }))
    (Routine_store.roster dirs)
...
let item ~kind ~name parts =
  Html.El.li
    ...
    (Html.El.span ~at:[ Html.At.class_ "routine" ] [ Html.El.txt name ]
...
let run_items ~name ~now ~budget runs =
  List.concat_map
    (fun { Observed.pending; fence } ->
      ...
      match fence with
      | `Held when now -. spawned_at > budget.Routine.Budget.wall_clock ->
          [
            ( rank_overdue,
...
(* One routine's attention items. Disposition-derived buckets fold under
      ...
      [
        ( rank_broken,
          item ~kind:"broken" ~name
            [ detail (Routine_store.Error.message error) ] );
      ]
  | Observed.Routine { Observed.loaded; receipts; runs } -> (
...
(* One roster row. A broken routine has no digest, state, or fences to
   show — its refusal is an attention item, so it takes no row here. *)
let record_row ~now ~ingress = function
  | Observed.Broken _ -> None
  | Observed.Routine { Observed.loaded; receipts; runs = _ } ->
      let name = loaded.Routine_store.Loaded.name in
      let digest = loaded.Routine_store.Loaded.digest in
      let routine = loaded.Routine_store.Loaded.routine in
      let budget = routine.Routine.budget in
      let state = if routine.Routine.enabled then "enabled" else "disabled" in
      let meters =
        match receipts with
        | Error _ ->
            [
...
let shell body =
  Page.document ~head_title:"Mentat — Routines"
    ~body:
      [
        Html.El.v
          ...
          [
            ...
            Html.El.h 1 [ Html.El.txt "Routines" ];
          ];
        Html.El.v ~at:[ Html.At.class_ "routines" ] "main" body;
      ]
      ...
      shell
        [
          Html.El.p
            ...
            [ Html.El.txt (Routine_store.Error.message error) ];
        ]
  | Ok [] ->
      shell
        [
      ...
      let record =
        Html.El.section
          ...
          | [] ->
              [
                empty_notice
                  "Every installed routine failed to load; each refusal is \
                   named above.";
              ]
