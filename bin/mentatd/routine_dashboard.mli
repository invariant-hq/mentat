...
(** The routines page — the needs-me-first dashboard on the daemon's web
    mount.
    ...
    outcome (the identity-scoped alerts the fire pipeline wrote), routines
    ...
    that sits the routine record, one row per loadable routine: state,
    ...
    nothing: installing, enabling, and removing routines stay with the
...
module Observed : sig
  type run = {
    pending : Mentat_routine.Receipt.Pending.t;
        (** A spawned disposition with no reaped line. *)
    fence : Mentat_routine.Record.fence;
...
(** [page ~now ~ingress observed] is the complete dashboard document for
    ...
    parked, failed, broken routines, unreadable receipts, overdue runs,
    ...
    per-routine record follows. An empty roster, an empty attention list,
    ...
    listener is up), prefixed onto each webhook routine's ingress path. *)
