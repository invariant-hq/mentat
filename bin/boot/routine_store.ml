...
let routine_json_name = "routine.json"
...
(* Policy files share the executable's small-document cap; the receipt log
   grows for the routine's life and gets a generous one. *)
...
let decode_routine ~operation ~path bytes =
  match Routine.decode bytes with
  | Ok routine -> Ok routine
  | Error e -> error ~operation ~path (Routine.Error.message e)
...
let load dirs ~name =
  ...
  let* () =
    ...
    else error ~operation ~path:dir (Printf.sprintf "no routine named %s" name)
  in
  let* () = require_private ~operation dir in
  let json_path = Filename.concat dir routine_json_name in
  let* routine_json = read_required ~operation json_path in
  let* routine = decode_routine ~operation ~path:json_path routine_json in
  let* () =
    if String.equal routine.Routine.name name then Ok ()
    else
      error ~operation ~path:json_path
        (Printf.sprintf "routine is named %S but its directory is %S"
           routine.Routine.name name)
  in
  let* prompt =
    read_required ~operation (Filename.concat dir routine.Routine.run.Routine.Run.prompt)
  in
  let* output_schema =
    read_required ~operation
      (Filename.concat dir routine.Routine.run.Routine.Run.output_schema)
  in
    ...
    if not (Option.is_some (Routine.webhook_arm routine)) then Ok ()
      ...
      | None, _ ->
          error ~operation ~path:(Filename.concat dir ingress_id_name)
            "a webhook routine has no ingress id; run `mentatd routine add` \
             to mint one"
      | Some _, false ->
          error ~operation ~path:secret_path
            "a webhook routine has no webhook secret; run `mentatd routine \
             add` to mint one"
  in
  let digest = Routine.policy_digest ~routine_json ~prompt ~output_schema in
  Ok { Loaded.name; dir; routine; digest; prompt; output_schema; ingress_id }

let roster dirs =
  let root = User_dirs.routines_dir dirs in
...
let claim_path dirs ~name ~digest identity =
  let key =
    Mentat_digest.key ~length:32 ~domain:"mentat.routine.event.v1"
      [ digest; Event.Identity.to_string identity ]
  in
  Filename.concat
    (Filename.concat (User_dirs.routine_state_dir dirs name) "events")
    key
...
let install dirs ~src =
  ...
  let* src_dir =
    ...
    | true -> Ok src
    | false ->
        if String.equal (Filename.basename src) routine_json_name then
          Ok (Filename.dirname src)
        else
          error ~operation ~path:src
            "expected a routine directory or its routine.json"
    | exception Sys_error reason -> error ~operation ~path:src reason
  in
  let json_path = Filename.concat src_dir routine_json_name in
  let* routine_json = read_required ~operation json_path in
  let* routine = decode_routine ~operation ~path:json_path routine_json in
  ...
  let* () =
    if Char.equal name.[0] '.' then
      error ~operation ~path:json_path
        (Printf.sprintf "routine name %S must not open with a dot" name)
    else Ok ()
  in
  let* prompt =
    read_required ~operation
      (Filename.concat src_dir routine.Routine.run.Routine.Run.prompt)
  in
  let* output_schema =
    read_required ~operation
      (Filename.concat src_dir routine.Routine.run.Routine.Run.output_schema)
  in
  let dst = User_dirs.routine_dir dirs name in
  let in_place =
    ...
    if in_place then Ok ()
      ...
      let foreign kind =
        error ~operation ~path:(Filename.concat src_dir kind)
          (Printf.sprintf
             "a routine proposal must not carry %s; it is created at install"
             kind)
      in
      let* () =
      ...
      let* () =
        atomic_write_wrapped ~operation
          (Filename.concat dst routine_json_name)
          routine_json
      in
      let* () =
        atomic_write_wrapped ~operation
          (Filename.concat dst routine.Routine.run.Routine.Run.prompt)
          prompt
      in
      atomic_write_wrapped ~operation
...
let rotate_webhook_secret (loaded : Loaded.t) =
  ...
  if not (Option.is_some (Routine.webhook_arm loaded.Loaded.routine)) then
    error ~operation ~path
      "the routine has no github_webhook trigger, so there is no webhook \
       secret to rotate"
  else
