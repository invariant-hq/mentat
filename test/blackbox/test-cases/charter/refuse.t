...
lenient read.
  ...
  mentat: unknown/charter.json: unknown member "bogus_member"
  [1]
...
from an unknown kind.
  ...
  mentat: sched/charter.json: trigger[0].kind: "schedule" parses but is refused as unimplemented
  [1]
...
every later load — refuses it, naming the mode.
  ...
  mentat: $TESTCASE_ROOT/config/mentat/charters/loose/secrets/webhook: mode 644 grants group or world access; make it private (chmod go-rwx)
  [1]
  $ mentat charter list | censor
  NAME   DIGEST  STATE  LAST
  loose  -       -      load error: $TESTCASE_ROOT/config/mentat/charters/loose/secrets/webhook: mode 644 grants group or world access; make it private (chmod go-rwx)
...
out of a repository checkout.
  ...
  mentat: smuggle/secrets: a charter proposal must not carry secrets; it is created at install
  [1]
