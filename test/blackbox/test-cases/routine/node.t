The resident routine node over the wire: mentatd binds the webhook ingress
on loopback, a routine installed while the daemon runs answers to its
...
--routine-git-base flag names a host prefix and the node derives each
routine's remote from the repository it watches.
...
to standard output — and the validated GitHub seams as flags. No routine is
installed yet.

  $ trap stop_daemon EXIT
  $ export MENTAT_DAEMON_MAX_IDLE=300
  $ mentatd --ingress-port 0 --github-base-url "$GH_URL" \
  >   --routine-git-base "$PWD/remotes" >daemon-serve.out 2>&1 &
  $ MENTAT_DAEMON_PID=$!
...
The routine is installed while the node runs — the file is the
...
receipt log speaks routine facts, and a foreign event kind is noted in the
trace log only.
...
provider through the sealed review contract.
  ...
  1
  $ grep -c 'routine pr-review: queued' daemon-serve.out
  1
