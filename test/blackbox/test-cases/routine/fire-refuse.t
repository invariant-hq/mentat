...
hook firing, and the verb's own ladder refuses a fire the routine's trigger
list does not admit or the credentials cannot serve. No provider, checkout,
or run is ever reached.

A routine metering one run per hour, alerting on fenced through a hook that
...
never claims, fences, or spawns.
  ...
  $ mentatd routine fire pr-review --event event-stale.json | censor
...
codec, so writing them directly is writing what the pipeline would have.
  ...
  $ digest=$(mentatd routine list | awk 'NR==2 {print $2}')
...
once, and the hook heard exactly one structured event.
  ...
  $ export MENTAT_GITHUB_BASE_URL="http://127.0.0.1:$(cat gh-port)"
  $ mentatd routine fire pr-review --event event-fresh.json | censor
...
line, no second hook firing.
  ...
  $ export MENTAT_GITHUB_BASE_URL="http://127.0.0.1:$(cat gh-port)"
  $ mentatd routine fire pr-review --event event-later.json | censor
...
without a webhook arm has no deliveries for --event or --sweep to replay.
  ...
  $ cat > hookless/routine.json <<'EOF'
  > { "routine": 1, "name": "hookless",
  >   "workspace": { "repo": "acme/widgets" },
  ...
  $ mentatd routine add hookless >/dev/null
  $ mentatd routine fire hookless --event event-fresh.json 2>&1
  mentat: routine hookless has no cli trigger arm; add {"kind": "cli"} to its trigger list to fire it by hand
  [2]
  $ cat > webhookless/routine.json <<'EOF'
  > { "routine": 1, "name": "webhookless",
  >   "workspace": { "repo": "acme/widgets" },
  >   "trigger": [ { "kind": "cli" } ],
  >   "run": { "mode": "review", "prompt": "prompt.md",
  >            "output_schema": "findings.schema.json" },
  ...
  $ mentatd routine add webhookless >/dev/null
  $ mentatd routine fire webhookless --event event-fresh.json 2>&1
  mentat: routine webhookless has no github_webhook trigger; --event and --sweep replay webhook deliveries
  [2]
  $ mentatd routine fire webhookless --sweep 2>&1
  mentat: routine webhookless has no github_webhook trigger; --event and --sweep replay webhook deliveries
  [2]

--event and --sweep are one choice, and a bare fire has nothing to review.

  $ mentatd routine fire pr-review --event event-fresh.json --sweep 2>&1
...
The webhook default: a routine metering nothing per-routine still fences at
...
unfenced webhook routine cannot exist by omission. Six in-window spawns are
seeded; the seventh admissible delivery is fenced.

  $ mkdir nometer
  $ cat > nometer/routine.json <<'EOF'
  > { "routine": 1, "name": "nometer",
  >   "workspace": { "repo": "acme/widgets" },
  >   "trigger": [
  >     { "kind": "github_webhook", "events": ["pull_request.opened"] },
  >     { "kind": "cli" } ],
  ...
  $ mentatd routine add nometer >/dev/null
  $ NDIR="$PWD/config/mentat/routines/nometer"
  $ printf 'test-read-token\n' > "$NDIR/secrets/read-token"
  $ chmod 600 "$NDIR/secrets/read-token"
  $ NRECEIPTS="$PWD/state/mentat/routines/nometer/receipts.jsonl"
  $ ndigest=$(mentatd routine list | awk '$1 == "nometer" {print $2}')
  ...
  $ export MENTAT_GITHUB_BASE_URL="http://127.0.0.1:$(cat gh-port)"
  $ mentatd routine fire nometer --event event-seventh.json | censor
  fenced github:acme/widgets#14@$DIGEST:head: runs_per_hour
  $ wait_fake_server
  $ mentatd routine status nometer | grep 'runs 1h'
    runs 1h: 6 of 6
...
freed, it drives on to the checkout. The receipts poison nothing.
  ...
  $ export MENTAT_GITHUB_BASE_URL="http://127.0.0.1:$(cat gh-port)"
  $ mentatd routine fire pr-review --sweep | censor
...
did not poison re-admission; only the run-claim dedups.
  ...
  $ export MENTAT_GITHUB_BASE_URL="http://127.0.0.1:$(cat gh-port)"
  $ export MENTAT_ROUTINE_GIT_URL="$PWD/no-such-remote"
  $ mentatd routine fire pr-review --sweep 2>&1 | censor | sed -e 's/checkout: .*/checkout: (git error)/' -e 's/^mentat: git .*/mentat: (git error)/'
...
freely once the routine is repaired. (Breaking the schema moves the policy
digest, so this delivery is a fresh identity under the edited policy.)

  $ ls "$PWD/state/mentat/routines/pr-review/events" | wc -l | tr -d ' '
  1
  $ ls "$HOME/.cache/mentat/routines/pr-review/runs" | wc -l | tr -d ' '
  1
  $ printf 'not json\n' > "$CDIR/findings.schema.json"
  $ start_fake_server github-fresh.jsonl capture-badschema gh-port
  $ export MENTAT_GITHUB_BASE_URL="http://127.0.0.1:$(cat gh-port)"
  $ mentatd routine fire pr-review --event event-fresh.json 2>&1 | censor | sed -e 's/not valid JSON: .*/not valid JSON: (parse error)/' -e '/^File "-"/d'
