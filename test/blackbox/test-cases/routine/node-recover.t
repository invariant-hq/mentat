...
re-enters the publisher only. With an enabled webhook routine installed the
idle watchdog stands down — the routine is a standing commission, so the
node stays resident where a daemon with no routines would stop itself.

The fixture: the fire.t repository, routine, and delivery — the remote laid
out under a git base so the daemon half can derive it per routine.
...
run child keeps the provider connection and settles on its own.
  ...
  $ export MENTAT_ROUTINE_GIT_URL="$PWD/remotes/acme/widgets.git"
...
notice on the harness's own stderr.
  ...
  $ FIRE_PID=$(mentatd routine fire pr-review --event event.json >fire.out 2>&1 & echo $!)
...
MENTAT_DAEMON_MAX_IDLE is one second — the enabled webhook routine pins
...
exists to keep a pass off a run whose activation is still staging.
  ...
  $ unset MENTAT_ROUTINE_GIT_URL
  $ trap stop_daemon EXIT
  $ export MENTAT_DAEMON_MAX_IDLE=1
  $ export MENTAT_ROUTINE_SPAWN_GRACE=0
  $ mentatd --github-base-url "$GH_URL" --routine-git-base "$PWD/remotes" \
...
minted no run and spent nothing — no provider fake is even alive.
  ...
  1
  $ mentatd routine runs pr-review | censor --times
...
one-second idle budget, a daemon with no routines would have stopped; the node
is still serving.
