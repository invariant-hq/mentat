...
The routine, installed, with both credentials in place.
...
routine's provenance and contract in its metadata, the trigger prompt as a
mailed queue entry, and the turn that consumed it as a triggered turn.
  ...
  1
  $ RUN_ROOT=$(ls -d "$HOME"/.cache/mentat/routines/pr-review/runs/*)
...
dedupes exactly as the webhook path would.
  ...
  $ export MENTAT_GITHUB_BASE_URL="http://127.0.0.1:$(cat gh-port)"
  $ mentatd routine fire pr-review --sweep
  $ wait_fake_server
...
receipt log stays intact.
  ...
  $ export MENTAT_GITHUB_BASE_URL="http://127.0.0.1:$(cat gh-port)"
  $ export MENTAT_ROUTINE_GIT_URL="$PWD/missing.git"
  $ mentatd routine fire pr-review --event event8.json 2>&1 | censor | sed -e 's/checkout: .*/checkout: (git error)/' -e 's/^mentat: git .*/mentat: (git error)/'
...
so the run settles with findings but no egress receipt.
  ...
  $ export MENTAT_GITHUB_BASE_URL="http://127.0.0.1:$(cat gh-port)"
  $ mentatd routine fire pr-review --event event8.json 2>&1 | censor
