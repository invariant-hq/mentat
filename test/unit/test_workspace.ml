...
let finding ?(lane = Workspace.Finding.Lane.Build)
    ?(severity = Workspace.Finding.Severity.Error) ?path ?location head =
  Workspace.Finding.v ~lane ~severity ?path ?location ~head ()

let classified ?(lint = true) ?(severity = Workspace.Finding.Severity.Warning)
    head =
  Workspace.Finding.classify ~lint ~severity ~head ()

let reading ?lint ?(empty_confirmed = true) findings =
  Workspace.Build_change.Reading.make
    ~build:(Workspace.Build_change.Reading.lane ~empty_confirmed findings)
    ?lint ()

let step state reading = Workspace.Build_change.step state reading

let titles changes =
  List.map
    (fun change -> Workspace.Notice.title (Workspace.Build_change.notice change))
    changes
    ...
    [
      test "the marker classifies, and only with a rule-shaped tail" (fun () ->
          ...
          is_true ~msg:"marker tail is lint"
            (Workspace.Finding.Lane.equal
               (Workspace.Finding.lane (classified lint_head))
               Workspace.Finding.Lane.Lint);
          List.iter
            (fun head ->
              is_true ~msg:("build lane: " ^ head)
                (Workspace.Finding.Lane.equal
                   (Workspace.Finding.lane (classified head))
                   Workspace.Finding.Lane.Build))
            [
          ...
          is_true ~msg:"lint:false keeps the marker in the build lane"
            (Workspace.Finding.Lane.equal
               (Workspace.Finding.lane (classified ~lint:false lint_head))
               Workspace.Finding.Lane.Build));
      test "identity is content: positions move silently, duplicates collapse"
        (fun () ->
          ...
          in
          let state = Workspace.Build_change.State.initial in
          let changes, state =
      ...
      test "lanes are independent and an absent lint lane is frozen" (fun () ->
          ...
          let changes, state =
            step state
              (Some
                 (reading
                    ~lint:(Workspace.Build_change.Reading.lane [ lint_finding ])
                    [ finding "boom" ]))
          in
      ...
      test "a lint lane reading empty and confirmed is lint clean" (fun () ->
          ...
          let changes, _ =
            step state
              (Some (reading ~lint:(Workspace.Build_change.Reading.lane []) []))
          in
      ...
      test "notice bytes: severity, title grammar, body, coalescing keys"
        (fun () ->
          ...
          let notice =
            Workspace.Build_change.notice (List.nth changes 0)
          in
          equal string ~msg:"source" "dune" (Workspace.Notice.source notice);
          is_true ~msg:"severity error"
            (Workspace.Notice.Severity.equal
               (Workspace.Notice.severity notice)
               Workspace.Notice.Severity.Error);
          equal string ~msg:"title" "Build failing (2 errors: 2 new)"
            (Workspace.Notice.title notice);
          equal (option string) ~msg:"body lists fresh findings"
            ...
            (Workspace.Notice.body notice);
          equal string ~msg:"key" "dune.build" (Workspace.Notice.key notice);
      ...
      test "a warnings-only build lane is failing at warning severity"
        (fun () ->
          let w =
            finding ~severity:Workspace.Finding.Severity.Warning
              ~path:"lib/a.ml" "unused open"
          in
          let changes, _ =
            step Workspace.Build_change.State.initial (Some (reading [ w ]))
          in
          let notice =
            Workspace.Build_change.notice (List.nth changes 0)
          in
      ...
      prop
        ...
        (fun heads ->
          ...
          in
          let _, state =
            step Workspace.Build_change.State.initial
