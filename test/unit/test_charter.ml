...
(* Unit suite for [Mentat_charter], the pure charter layer: the strict
   version-1 charter decode with its closed grant envelope, the policy
   ...
   The modules live in the private [mentat_charter] library under
   [bin/charter/]. *)
...
(* The reference charter: a webhook-and-cli review charter exercising every
...
let charter_decodes () =
  ...
  | Ok c -> (
      equal string ~msg:"name" "pr-review" c.Charter.name;
      is_true ~msg:"enabled" c.Charter.enabled;
      equal string ~msg:"repo" "invariant/spice" c.Charter.repo;
      (match c.Charter.triggers with
      | [ Charter.Trigger.Github_webhook webhook; Charter.Trigger.Cli ] ->
          equal (list string) ~msg:"events"
            ...
            webhook.Charter.Trigger.Webhook.events;
          let gate = webhook.Charter.Trigger.Webhook.gate in
          equal
            (option (list string))
            ~msg:"gate base" (Some [ "main" ]) gate.Charter.Gate.base;
          is_false ~msg:"gate drafts" gate.Charter.Gate.drafts;
          equal
            (option (list string))
            ~msg:"gate associations"
            (Some [ "OWNER"; "MEMBER"; "COLLABORATOR" ])
            gate.Charter.Gate.associations
      | _ -> fail "expected a webhook arm then a cli arm");
      equal (option string) ~msg:"model" (Some "claude-sonnet-4-6")
        c.Charter.run.Charter.Run.model;
      equal (option string) ~msg:"reasoning" (Some "high")
        c.Charter.run.Charter.Run.reasoning;
      equal int ~msg:"max_steps" 60 c.Charter.run.Charter.Run.max_steps;
      equal string ~msg:"prompt" "prompt.md" c.Charter.run.Charter.Run.prompt;
      equal string ~msg:"output_schema" "findings.schema.json"
        c.Charter.run.Charter.Run.output_schema;
      equal (option bool) ~msg:"project_instructions" None
        c.Charter.run.Charter.Run.project_instructions;
      equal float_exact ~msg:"wall_clock is 15 minutes of seconds" 900.0
        c.Charter.budget.Charter.Budget.wall_clock;
      equal (option float_exact) ~msg:"usd_per_day" (Some 15.0)
        c.Charter.budget.Charter.Budget.usd_per_day;
      equal (option int) ~msg:"runs_per_hour" (Some 6)
        c.Charter.budget.Charter.Budget.runs_per_hour;
      (match c.Charter.notify with
      | Some notify ->
          equal (list string) ~msg:"notify.on"
            [ "failed"; "parked"; "fenced" ]
            (List.map Receipt.Transition.to_string notify.Charter.Notify.on);
...
(* A minimal charter: only the required members, so every default shows. *)
let minimal =
  {|{
  "charter": 1,
  "name": "nightly",
...
let charter_defaults () =
  match Charter.decode minimal with
  | Error e -> failf "decode: %s" (Charter.Error.message e)
  | Ok c ->
      is_true ~msg:"enabled defaults true" c.Charter.enabled;
      equal int ~msg:"max_steps defaults to 60" 60
        c.Charter.run.Charter.Run.max_steps;
...
let gate_defaults_when_omitted () =
  ...
  in
  match Charter.decode payload with
  | Error e -> failf "decode: %s" (Charter.Error.message e)
  | Ok c -> (
      match c.Charter.triggers with
      | [ Charter.Trigger.Github_webhook webhook ] ->
          let gate = webhook.Charter.Trigger.Webhook.gate in
          equal (option (list string)) ~msg:"any base" None
            gate.Charter.Gate.base;
          is_false ~msg:"drafts refused by default" gate.Charter.Gate.drafts;
          equal (option (list string)) ~msg:"any author" None
            gate.Charter.Gate.associations
      | _ -> fail "expected exactly the webhook arm")

(* Charter payload surgery: a fixture with one member line replaced, so
...
let rejects ~msg ?expect payload =
  ...
  | Error e -> (
      let message = Charter.Error.message e in
      match expect with
      | None -> ()
      | Some needle ->
...
let charter_envelope_is_closed () =
  ...
  rejects ~msg:"future version" ~expect:"unsupported version 2"
    (amended ~old_string:{|"charter": 1|} ~new_string:{|  "charter": 2,|});
  rejects ~msg:"goal does not exist" ~expect:{|unknown member "goal"|}
    (amended ~old_string:{|"charter": 1|}
       ~new_string:{|  "charter": 1, "goal": "ship it",|});
  rejects ~msg:"write sandbox refused" ~expect:{|only "read-only"|}
    (amended ~old_string:{|"charter": 1|}
       ~new_string:{|  "charter": 1, "sandbox": "workspace-write",|});
  rejects ~msg:"unattended review refused" ~expect:{|"deny" or "block"|}
    (amended ~old_string:{|"charter": 1|}
       ~new_string:{|  "charter": 1, "permission_unattended": "review",|});
  ...
  rejects ~msg:"prompt naming a reserved file" ~expect:"run.prompt"
    (amended ~old_string:{|"prompt": "prompt.md",|}
       ~new_string:{|    "prompt": "charter.json",|});
  ...
  rejects ~msg:"negative usd fence" ~expect:"budget.per_charter.usd_per_day"
    (amended
       ~old_string:{|"per_charter": { "usd_per_day": 15.0, "runs_per_hour": 6 } },|}
       ~new_string:
         {|              "per_charter": { "usd_per_day": -1.0, "runs_per_hour": 6 } },|});
  rejects ~msg:"zero rate fence" ~expect:"budget.per_charter.runs_per_hour"
    (amended
       ~old_string:{|"per_charter": { "usd_per_day": 15.0, "runs_per_hour": 6 } },|}
       ~new_string:
         {|              "per_charter": { "usd_per_day": 15.0, "runs_per_hour": 0 } },|})

let charter_trigger_kinds () =
  let with_arm arm =
...
let policy_digest_identity () =
  let digest =
    Charter.policy_digest ~charter_json:conforming ~prompt:"Review the diff."
      ~output_schema:"{}"
  in
  ...
  equal string ~msg:"pinned derivation" "f37693004d6b3887" digest;
  equal string ~msg:"deterministic" digest
    (Charter.policy_digest ~charter_json:conforming
       ~prompt:"Review the diff." ~output_schema:"{}");
  let differs ~msg other = is_true ~msg (not (String.equal digest other)) in
  differs ~msg:"charter.json moves the digest"
    (Charter.policy_digest ~charter_json:(conforming ^ " ")
       ~prompt:"Review the diff." ~output_schema:"{}");
  ...
  is_true ~msg:"framing forbids cross-file byte shifts"
    (not
       (String.equal
          (Charter.policy_digest ~charter_json:"a" ~prompt:"bc"
             ~output_schema:"d")
          (Charter.policy_digest ~charter_json:"ab" ~prompt:"c"
             ~output_schema:"d")))
...
let webhook_arm =
  {
    Charter.Trigger.Webhook.events =
      [ "pull_request.opened"; "pull_request.synchronize" ];
    gate =
      {
        Charter.Gate.base = Some [ "main" ];
        drafts = false;
    ...
    (Gate.evaluate ~repo
       {
         ...
         Charter.Trigger.Webhook.gate =
           { webhook_arm.Charter.Trigger.Webhook.gate with Charter.Gate.drafts = true };
       }
       { sample_event with Event.Pull_request.draft = true });
...
let run_id_mint () =
  ...
  equal string ~msg:"pinned derivation" "c-fdfec12877f34773" id;
...
let () =
  run "mentat.charter"
    [
      test "the reference charter decodes fully" charter_decodes;
      test "omitted members take their documented defaults" charter_defaults;
