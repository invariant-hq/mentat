...
(* Unit suite for [Routine_dashboard]'s pure projection — the needs-me-first
...
(* Routine fixtures, built directly — no disk. *)
...
let test_routine ?(enabled = true) ?(webhook = false) ?usd_per_day
    ?runs_per_hour ?(wall_clock = 900.) name =
  {
    Routine.name;
    enabled;
    repo = "acme/widgets";
    triggers = (if webhook then [ webhook_arm ] else [ Routine.Trigger.Cli ]);
    ...
    budget = { Routine.Budget.wall_clock; usd_per_day; runs_per_hour };
    notify = None;
    suppress_clean_run = false;
  }

let loaded ?(digest = "aaaaaaaaaaaaaaaa") ?ingress_id routine =
  {
    Routine_store.Loaded.name = routine.Routine.name;
    dir = "/nonexistent/" ^ routine.Routine.name;
    routine;
    digest;
    prompt = "";
    output_schema = "";
...
let observed ?(receipts = Ok []) ?(runs = []) loaded =
  Routine_dashboard.Observed.Routine
    { Routine_dashboard.Observed.loaded; receipts; runs }

let open_run ~fence (pending : Receipt.Pending.t) =
  { Routine_dashboard.Observed.pending; fence }
...
let store_error message =
  { Routine_store.Error.operation = "read"; path = "/nonexistent"; reason = message }

let html ?(ingress = None) observed =
  Mentat_web.Html.to_string (Routine_dashboard.page ~now ~ingress observed)

(* The empty and failure statements: never a blank page. *)

let empty_roster () =
  let page = html (Ok []) in
  contains ~msg:page ~sub:"No routines are installed" page;
  contains ~sub:"mentatd routine add" page
...
let nothing_needed () =
  let page =
    html (Ok [ observed (loaded (test_routine ~usd_per_day:15.0 "quiet")) ])
  in
...
let bucket_order () =
  ...
  let fenced_routine =
    let routine = test_routine ~webhook:true ~runs_per_hour:1 "g-fenced" in
    observed
      ~receipts:(Ok [ spawned ~identity:"i-g" ~digest:d "s-g" ])
      (loaded ~digest:d routine)
  in
  let entries =
    [
      observed
        ...
        (loaded ~digest:d (test_routine "a-healthy"));
      observed
        ~receipts:
          (Ok
             [
        ...
        (loaded ~digest:d (test_routine "b-failed"));
      observed
        ~receipts:
          (Ok
             [
        ...
        (loaded ~digest:d (test_routine "c-parked"));
      Routine_dashboard.Observed.Broken
        { name = "d-broken"; error = store_error "bad json" };
      observed
        ~receipts:(Error (store_error "torn line"))
        (loaded ~digest:d (test_routine "e-unreadable"));
      observed
        ...
        (loaded ~digest:d (test_routine ~wall_clock:900. "f-overdue"));
      fenced_routine;
      observed
        ~receipts:
          (Ok
      ...
      (Ok
         [
           observed
             ...
             (loaded ~digest:d (test_routine "pr-review"));
         ])
  in
      ...
      (Ok
         [
           observed
             ...
             (loaded ~digest:d (test_routine ~wall_clock:900. "late"));
         ])
  in
      ...
      (Ok
         [
           observed
             ...
             (loaded ~digest:d (test_routine ~wall_clock:900. "live"));
         ])
  in
      ...
      (Ok
         [
           observed
             ...
             (loaded ~digest:d (test_routine "dead"));
         ])
  in
      ...
      (Ok
         [
           observed
             ...
             (loaded ~digest:d (test_routine ~usd_per_day:10.0 "spent"));
         ])
  in
         ...
         [
           observed
             ...
             (loaded ~digest:"aaaaaaaaaaaaaaaa"
                (test_routine ~wall_clock:900. "edited"));
         ])
  in
      ...
      (Ok
         [
           observed
             ...
             (loaded ~digest:d (test_routine "owed"));
         ])
  in
  contains ~msg:page
...
let broken_only_roster () =
  ...
  contains ~sub:"Every installed routine failed to load" page;
...
let ingress_lines () =
  let routine =
    loaded ~ingress_id:"0123456789abcdef0123456789abcdef"
      (test_routine ~webhook:true "hooked")
  in
  let bound = html ~ingress:(Some "127.0.0.1:7777") (Ok [ observed routine ]) in
  contains ~msg:bound
    ~sub:
      "ingress: POST http://127.0.0.1:7777/ingress/github/0123456789abcdef0123456789abcdef"
    bound;
  let unbound = html (Ok [ observed routine ]) in
  contains ~msg:unbound
    ~sub:
      "ingress: POST /ingress/github/0123456789abcdef0123456789abcdef (unbound)"
    unbound;
  let cli = html (Ok [ observed (loaded (test_routine "plain")) ]) in
      ...
      (Ok
         [
           observed
             ...
             (loaded ~digest:d
                (test_routine ~webhook:true ~usd_per_day:15.0 ~runs_per_hour:6
